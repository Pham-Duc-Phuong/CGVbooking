...
export const BookingTemplate = () => {
    ...
    return (
        <div className="max-w-screen-2xl m-auto px-[10px] md:px-[30px] py-[30px] sm:py-[60px] ">
            <button className="btn-reset flex items-center justify-center font-[400] mb-2 sm:mb-4 px-4 py-1 text-[11px] sm:text-[14px]" onClick={() => { window.history.back() }}><img className="w-[20px] sm:w-[25px] mr-1" src="/images/popcorn-svgrepo-com.svg"></img>Quay lại</button>
            <div className="flex flex-col lg:grid lg:grid-cols-booking lg:gap-[10px]">
                ...
                <Modal footer={false} closeIcon={false} open={payBankType} onCancel={() => { setPayBankType(false) }}>
                    <button type="button" className="absolute top-[25px] right-[30px] btn-reset p-2 sm:p-3" onClick={() => { setPayBankType(false) }}>
                        {xIconSVG()}
                        <span className="sr-only">Close modal</span>
                    </button>
                    <img src="/images/TCB-QR-PHAM DUC PHUONG-2023924214542.jpg" alt="" />
                </Modal>
                <Modal footer={false} closeIcon={false} open={chonGhe} onCancel={() => { setChonGhe(false) }}>
                    <div className="p-6 text-center">
                        ...
                        <button data-modal-hide="popup-modal" type="button" className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2" onClick={() => { setChonGhe(false) }}>
                            Chọn vị trí ngồi
                        </button>
                    </div>
                </Modal>
                <Modal footer={false} closeIcon={false} open={payType} onCancel={() => { setPayType(false) }}>
                    <div className="p-6 text-center">
                        <svg className="mx-auto mb-4 text-white w-12 h-12 dark:text-gray-200" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                            <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 11V6m0 8h.01M19 10a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        </svg>
                    ...
                    <button className="btn-cancel" onClick={() => { setPayType(false) }}>Chọn phương thức thanh toán</button>
                </Modal>
                <Modal footer={false} closeIcon={false} open={isModalOpen} onCancel={() => { setIsModalOpen(false) }}>
                    <button type="button" className="absolute top-3 right-2.5 btn-reset p-2 sm:p-3 from-orange-400 to-red-600" onClick={() => { setIsModalOpen(false) }}>
                        {xIconSVG()}
                        <span className="sr-only">Close modal</span>
                    </button>
                    {/* Modal header */}
                    ...
                    <div className="px-0 py-2 sm:p-6 ">
                        ...
                        <ul className="my-2 sm:my-4 space-y-3 ">
                            <li>
                                <button className="payType" onClick={() => { setKhuyenMai(1), setPayBankType(true) }}>
                        ...
                        <div className="flex p-2 sm:p-4">
                            ...
                            <button className="btn-cancel !m-0 !mx-4" onClick={() => { setIsModalOpen(false) }}>Thoát</button>
                        </div>
                    </div>
                </Modal>
            </div>
